...
def translate_text(text):
    # Tradução simulada (substituir por API real se necessário)
    return (
        text.replace("The", "O").replace("Music", "Música")
            .replace("video", "vídeo").replace("fans", "fãs")
            .replace("album", "álbum").replace("news", "notícias")
            .replace("song", "canção").replace("performance", "apresentação")
            .replace("concert", "show")
    )
...
<rss version="2.0">
  <channel>
    <title>Notícias de Música – Billboard e NME</title>
...
def news_rss():
    ...
    for source, url in FEEDS.items():
        ...
        for entry in feed.entries[:10]:
            title = mock_translate(entry.title)
            description = mock_translate(entry.get("summary", ""))
            pubdate = brasilia_now().strftime("%a, %d %b %Y %H:%M:%S %z")
            items.append(f"""
            <item>
                <title>{escape(title)}</title>
                <description>{escape(source + ": " + description)}</description>
